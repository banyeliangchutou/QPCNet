...
model = QHybridClassifier(num_classes=10, input_size=(28, 28), batch_size=64).to(device)
...
with open("epoch_prf_log_fmnist.txt", "w") as f:
...
for epoch in range(num_epochs):
    ...
    with open("epoch_prf_log_fmnist.txt", "a") as f:
...
# plt.tight_layout()
# plt.savefig("training_results_fmnist.png")
...
# plt.savefig("confusion_matrix_fmnist.png")

# np.savetxt("train_losses_fmnist.txt", np.array(train_losses), fmt="%.6f")
# np.savetxt("test_accuracies_fmnist.txt", np.array(test_accuracies), fmt="%.2f")
